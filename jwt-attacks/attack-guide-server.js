...
If the server does not verify it — or trusts the alg header — it is useless.</pre>
  ...
  <div class="credentials-panel" style="margin-top:2rem">
    ...
    <div style="margin-top:0.6rem;display:flex;gap:0.6rem;flex-wrap:wrap">
      <button class="demo-btn" id="btn-test-forged-vulnerable">Test on Vulnerable :${VICTIM_PORT}</button>
      <button class="demo-btn" id="btn-test-forged-protected">Test on Protected :${PROTECTED_PORT}</button>
    </div>
...
// → accepts the token, grants admin access</pre>
  ...
  <div class="credentials-panel" style="margin-top:2rem">
    ...
    <div style="margin-top:0.6rem;display:flex;gap:0.6rem;flex-wrap:wrap">
      <button class="demo-btn" id="btn-test-cracked-vulnerable">Test on Vulnerable :${VICTIM_PORT}</button>
      <button class="demo-btn" id="btn-test-cracked-protected">Test on Protected :${PROTECTED_PORT}</button>
    </div>
...
// jwt.decode() is only for reading a token you've already verified</pre>
  ...
  <div class="credentials-panel" style="margin-top:2rem">
    ...
    <p style="font-size:0.85rem;color:#94a3b8;margin-bottom:0.75rem;max-width:720px">
      Both servers now have a Logout button. Try this sequence to see the revocation mechanism — and its limits.
    </p>
    <div class="flow-box">
...
5. Try the same old token again → it works — the denylist was in memory, now gone</pre>
    ...
    <div style="margin-top:1rem;display:flex;gap:0.6rem;flex-wrap:wrap;align-items:center">
      <button class="demo-btn" id="btn-test-revoked">Test Revoked Token on :${VICTIM_PORT}</button>
      <span style="font-size:0.8rem;color:#64748b">(paste the old token in Step 0 first)</span>
    </div>
    <div class="result-banner" id="revoked-result"></div>

    <div class="flow-box" style="margin-top:1rem">
      <strong>WHY THIS IS HARDER THAN IT LOOKS</strong><br><br>
      <pre>Stateless JWT (no logout):
  Client ──login──▶ Server issues token
  Client ──request with token──▶ Server verifies signature → ✓ done
  Client ──logout──▶ Server clears local storage → token still valid elsewhere

Denylist approach (what we added):
  POST /api/logout → server adds jti to denylist Set
  Every request: server checks denylist after signature verification
  Problem: Set lives in memory → lost on restart
           Multiple server instances each have their own Set → inconsistent
  Fix: use Redis (shared, persistent) → but now JWT is stateful

Short-lived tokens + refresh tokens (production approach):
  access token: 15 min exp — short enough that "logout" matters less
  refresh token: 7 days, stored in HttpOnly cookie, can be revoked in DB
  On logout: revoke the refresh token → new access tokens cannot be issued
  Old access tokens expire on their own within 15 minutes

TL;DR: stateless JWTs cannot be truly revoked without server state.
       If you need instant revocation, you need either very short exp
       or a shared denylist (Redis). At that point, traditional
       session cookies might be simpler.</pre>
    </div>
  </div>
  ...
  <script>
    ...
    document.getElementById('btn-test-forged-vulnerable').addEventListener('click', function () {
      testToken('forged-token', 'forge-result', ${VICTIM_PORT});
    });
    document.getElementById('btn-test-forged-protected').addEventListener('click', function () {
      testToken('forged-token', 'forge-result', ${PROTECTED_PORT});
    });
    ...
    document.getElementById('btn-test-cracked-vulnerable').addEventListener('click', function () {
      testToken('cracked-token', 'cracked-test-result', ${VICTIM_PORT});
    });
    document.getElementById('btn-test-cracked-protected').addEventListener('click', function () {
      testToken('cracked-token', 'cracked-test-result', ${PROTECTED_PORT});
    });
    ...
    document.getElementById('btn-test-revoked').addEventListener('click', async function () {
      ...
      try {
        var res = await fetch('http://localhost:${VICTIM_PORT}/api/whoami', {
          headers: { 'Authorization': 'Bearer ' + token }
        });
        var data = await res.json();
        if (res.ok) {
          showBanner('revoked-result', 'info', 'ℹ Token still valid on :${VICTIM_PORT} — ' + JSON.stringify(data));
        } else {
