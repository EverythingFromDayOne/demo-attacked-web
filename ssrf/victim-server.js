 ...
 * How to Run:
 *
 * Terminal 1: cd demo-attacked/ssrf && npm install && npm run vulnerable
 * Terminal 2: cd demo-attacked/ssrf && npm run guide
 *
 * Attack sequence:
 * 1. http://localhost:3020  ← SSRF attack console — copy internal URLs
 * 2. http://localhost:3019  ← DevShare — paste URL into preview form
 * 3. Server fetches internal API and returns secrets in the preview card
 */

const express = require('express');
const cookieParser = require('cookie-parser');
const { buildPreviewFromFetch } = require('./preview-utils');
...
app.listen(PORT, function () {
  console.log('DevShare (VULNERABLE) running at http://localhost:' + PORT);
});
