...
| Server type | Theme |
|-------------|-------|
| Attacker server (cookie collector / dashboard) | Dark terminal aesthetic — exact CSS below. HTML lives in `public/attacker.html` (or `guide.html`), served via `res.sendFile`. |
| Victim servers | Realistic product UI matching their brand (NovaCRM, ShopNest, ConnectHub) |

**Attacker page — exact CSS (use these values, do not paraphrase or approximate):**
```css
body {
  background: #0a0a0a;
  color: #00ff41;
...
```
Only `<p>` text elements and `<input>` fields may use `max-width`. Never on `.flow-box`, `.credentials-panel`, or top-level containers. This is the canonical style every other attack demo in this lab also uses — apply it exactly to `stored/attacker.html`, `reflected/attacker.html`, and `svg-upload/attacker.html` (their two-panel layouts and victim switchers described below sit on top of this base CSS, unchanged).

---

## Code Comment Standard — educational depth, not one-liners

Comments on vulnerable and protected lines are teaching material, not labels. Each comment must answer: what is wrong/fixed, why it is exploitable/safe, how the attack works mechanically, and any nuance a student would miss without running the demo. Never write a single-line summary when a multi-line explanation teaches more.

Bad: `// ⚠️ VULNERABILITY: innerHTML renders user input`
Good:
```js
// ⚠️  VULNERABILITY: innerHTML renders raw HTML — never do this with user input.
// Any payload like <img src=x onerror="..."> executes immediately when the
// browser parses it as markup instead of text.
// Safe alternative: element.textContent = ticket.message
message.innerHTML = ticket.message;
```

Format: `// ⚠️ VULNERABILITY: <what and why>` above vulnerable lines, `// ✅ PROTECTED: <what was changed and why it works>` above fixed lines, in victim server/HTML files only — never in `attacker-server.js` or `attacker.html`.

**Required nuance for XSS (all three variants):** explain the difference between `innerHTML` (executes scripts and event handlers) and `textContent` / proper encoding (treats everything as literal text). For SVG upload specifically: explain that SVG is XML that can contain inline `<script>` tags, and that browsers sandbox SVG scripts inside `<img>` but not when the file is opened as a standalone document — this is a file-serving policy issue, not a frontend mistake.
...
### Canonical README structure (required — write directly in this order)

Write each variant README (`stored/README.md`, `reflected/README.md`, `svg-upload/README.md`)
directly in this exact section order, separated by `---` dividers between every
top-level section. Do not produce a different order — there is no later restructure pass.

```
# [Variant Name] — [App Name]
...
## Why It Works
## Defense Details
[optional: Payload Variants, Edge Cases, This Demo in Real Frameworks]
```

Rename mapping — use the canonical name directly, do not write the old name:
- "What It Is" / "Why These Lines Are Dangerous" → `## Why It Works`
- "Vulnerable Code" → `## Vulnerable Lines`
- "The Fix (exact lines)" → `## The Fix`
- The numbered "How to Run" steps described below go under `## How to Run` (run instructions only); split out any step that verifies the *protected* server into its own `## Protected Demo` section placed immediately after `## Attack Walkthrough`.

No `## Credentials` section is needed for XSS — these demos use a session cookie, not a login form.
