 ...
 *
 * The search term is still reflected in the page, but HTML-encoded so scripts
 * cannot execute. HttpOnly prevents JavaScript from reading the session cookie.
 */
...
<body>
  <div class="demo-banner">
    ✅ Protected: session cookie is HttpOnly — <strong id="cookie-display"></strong>
  </div>
  <header>
  ...
  <main>
    <div class="protected-note">
      Search term reflected safely via HTML encoding. Payloads display as plain text and cannot execute.
    </div>
  ...
  <script>
    var visible = document.cookie || '(empty — HttpOnly cookie hidden from JavaScript)';
    document.getElementById('cookie-display').textContent = visible;
  </script>
</body>
</html>`;
...
app.listen(PORT, () => {
  console.log(`ShopNest PROTECTED server running on http://localhost:${PORT}`);
  console.log(`Safe reflection demo: http://localhost:${PORT}/search?q=<script>alert(1)</script>`);
  console.log(`(Compare with vulnerable server on http://localhost:3004)`);
});
